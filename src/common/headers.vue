        ...
        <div class="nav-sub" :class="{ fixed: st }">
          ...
          <div class="nav-sub-wrapper" :class="{ fixed: st }">
            <div class="w">
              <ul class="nav-list2">
                <li>
                  <a to="/" @click="See(this.noticeUrl)"
                    ><strong>公告通知</strong></a
                  >
                </li>
                <li>
                  <a to="/" @click="See(this.topNewsUrl)"
                    ><strong>最新事件</strong></a
                  >
                </li>
                <li>
                  <a to="/" @click="See(this.newContentUrl)"
                    ><strong>最新研究</strong></a
                  >
                </li>
                <li>
                  <a to="/" @click="See(columnLinkUrl)"
                    ><strong>任务大厅</strong></a
                  >
                </li>
                <li>
                  <a to="/" @click="See(columnLinkUrl)"
                ...
                <li>
                  <a to="/" @click="See(columnLinkUrl)"
                    ><strong>知识库用户 </strong></a
                  >
                </li>
                <li>
                  <a to="/" @click="See(columnLinkUrl)"
                    ><strong>下载中心</strong></a
                  >
                </li>
                <li>
                  <a to="/" @click="See(columnLinkUrl)"
                    ><strong>帮助中心 </strong></a
                  >
                </li>
                <li>
                  <a to="/" @click="See(columnLinkUrl)"
...
export default {
  created() {
    this.getTopNews() // 最新事件
    this.getNewContent() // 最新研究内容
    this.getNotice() // 公告
  },
  data() {
    return {
      ...
      noticeUrl: '', // 通告
      topNewsUrl: '', // 最新事件
      newContentUrl: '', // 最新研究内容
      // 新版头部 结束
      columnLinkUrl: '', // 顶部快速跳转标题链接
      userName: '',
      user: {},
      // 查询数据库的商品
      st: false,
  ...
  methods: {
    ...
    getTopNews() {
      ...
      axios({
        ...
        this.topNewsUrl = res.data[0].columnLinkUrl
        let url = this.topNewsUrl.split('/html/')
        let u = url[1].split('/')
        let a = u[0]
        if (a === '') {
          a = u[1]
        }
        this.topNewsUrl = url[0] + '/' + a + '/index.html'
      })
    },
    // 最新研究内容
    getNewContent() {
      ...
      axios({
        ...
        this.newContentUrl = res.data[0].columnLinkUrl
        let url = this.newContentUrl.split('/html/')
        let u = url[1].split('/')
        let a = u[0]
        if (a === '') {
          a = u[1]
        }
        this.newContentUrl = url[0] + '/' + a + '/index.html'
      })
    },
    // 公告
    getNotice() {
      ...
      axios({
        ...
        this.noticeUrl = res.data[0].columnLinkUrl
        let url = this.noticeUrl.split('/html/')
        let u = url[1].split('/')
        let a = u[0]
        if (a === '') {
          a = u[1]
        }
        this.noticeUrl = url[0] + '/' + a + '/index.html'
      })
    },
